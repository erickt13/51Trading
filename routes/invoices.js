...
router.get('/', async (req,res) => {
    ...
    try {
        const invoices = await Invoice.find(searchOptions).populate('customer').populate('items').sort({ date: -1, invoiceNumber: -1 });
        console.log(invoices.map(inv => ({ number: inv.invoiceNumber, date: inv.date })));
